...
def compare_recur(co1, co2, is_dir=False):
    ...
    if co1['hash'] == co2['hash'] and co1['namehash'] == co2['namehash']:
        # No need to check!
        return
    print('M    {}{}'.format(co1['dirname'], '/' if is_dir else ''))
    ...
    for keyname in ['subfiles', 'subdirs']:
        ...
        for common_dirname in sorted(co1_sf_dict.keys() & co2_sf_dict.keys()):
            compare_recur(co1_sf_dict[common_dirname], co2_sf_dict[common_dirname], is_dir=keyname == 'subdirs')
        for co1_dir in sorted(co1_sf_dict.keys() - co2_sf_dict.keys()):
            print('A    {}{}'.format(co1_dir, '/' if keyname == 'subdirs' else ''))
            print_verbose('= {} differs ='.format(co1_dir))
            print_verbose('{}: {} in 1 but not 2'.format(keyname, co1_dir))
        for co2_dir in sorted(co2_sf_dict.keys() - co1_sf_dict.keys()):
            print('D    {}{}'.format(co2_dir, '/' if keyname == 'subdirs' else ''))
